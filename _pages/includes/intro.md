...
💼 参与快手推荐推理基座模型全流程开发：经历多轮 RFT、GRPO、OPSD 等策略与多阶段训练迭代，按日迭代训练与评测，使用千卡级集群协同推进；**负责核心 CoT 蒸馏方向**，深入定位 think &lt; non-think 性能瓶颈，设计并验证 CoT 构建方案，所构建的 CoT 为组内唯一有效版本，最终被论文采用；后续进一步研发改进版 CoT，经多轮验证性能较当前内部版本提升约 80%，不到一个月取得行业领先结果
</div>
</div>
